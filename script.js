...
function creerModal() {
  ...
  modal.innerHTML = `
    <div id="modal-contenu" style="
      background: var(--paper); max-width: 800px; width: 100%;
    ...
    ">
      ...
      <div id="modal-img-wrap" style="
        ...
        background: var(--card-bg); height: 560px;
        display: flex; align-items: center; justify-content: center;
      ">
        ...
        <div id="zoom-bar" style="
          ...
          background: rgba(23,19,15,0.7); padding: 0.4rem 0.6rem;
  ...
  function clampPan() {
    ...
    const maxX = Math.max(0, (imgW * zoomLevel - wrapW) / 2);
    const maxY = Math.max(0, (imgH * zoomLevel - wrapH) / 2);
  ...
  // Zoom à la molette
  document.getElementById('modal-img-wrap').addEventListener('wheel', e => {
    e.preventDefault();
    const ancien = zoomLevel;
    zoomLevel = e.deltaY < 0
      ? Math.min(zoomMax, +(zoomLevel + zoomStep / 2).toFixed(2))
      : Math.max(zoomMin, +(zoomLevel - zoomStep / 2).toFixed(2));
    if (zoomLevel === zoomMin) { panX = 0; panY = 0; }
    else if (ancien !== zoomLevel) {
      // ajuste le pan proportionnellement pour garder le même centre visuel
      const ratio = zoomLevel / ancien;
...
function btnStyle(outline) {
  return outline
    ? `display:inline-flex;align-items:center;gap:0.5rem;font-size:0.75rem;letter-spacing:0.05em;text-transform:uppercase;font-weight:700;color:var(--ink);text-decoration:none;background:var(--paper);border:2px solid var(--ink);box-shadow:4px 4px 0 var(--shadow-col);padding:0.75rem 1.5rem;transition:transform 0.15s,box-shadow 0.15s;font-family:'Space Grotesk',sans-serif;`
    : `display:inline-flex;align-items:center;gap:0.5rem;font-size:0.75rem;letter-spacing:0.05em;text-transform:uppercase;font-weight:700;background:var(--pink);color:var(--paper);text-decoration:none;border:2px solid var(--ink);box-shadow:4px 4px 0 var(--shadow-col);padding:0.75rem 1.5rem;transition:transform 0.15s,box-shadow 0.15s;font-family:'Space Grotesk',sans-serif;`;
}
